...
for ($i=0;$i<=@match_len-1 ;$i++) {
	...
	if ($match_total!=$temp[10]) {$identity=$identity_lixin;}#如果比到多段且有overlap，重新计算identity，以前的会低估

#if($name eq 'BY182479,1'){print "$match_total\t$size0\t$identity\t$identity_lixin\n";}


	$best = $exact;	#按精确比到的最长的作为最好的
	#print "$best_identity{$name}\t$identity\n";
	if ($identity < 70) {
		#print "$name\t$rough\t$cutoff\t$identity\n";
		next;}
	if(!defined($best{$name}) || $best{$name} < $best )
	{
		$best{$name} = $best;
		#$best_identity{$name}=$identity;
		$best_result{$name} = $_;
