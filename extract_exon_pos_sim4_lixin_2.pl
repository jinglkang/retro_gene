...
while(<I>){
	...
	if(/^seq1 \= /){
		...
		#print "$seq1_name------------------\n";
			my @SubjectPos_sort = sort {$a<=>$b} keys %sub_posi;#得到按起始位置排序的数组
			...
			my $id=1;
			for (my $i=0;$i<@SubjectPos_sort;$i++){
				
				my $len = $sub_posi{$SubjectPos_sort[$i]} - $SubjectPos_sort[$i] + 1;
									

				print O "$seq1_name$id\t$seq2_name\t$strand\t$SubjectPos_sort[$i]\t";$id++;
				print O "$sub_posi{$SubjectPos_sort[$i]}\t$len\t$query_start{$SubjectPos_sort[$i]}\t$query_end{$SubjectPos_sort[$i]}\n";	
