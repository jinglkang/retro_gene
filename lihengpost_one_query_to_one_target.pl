...
#print "$match_total\t$identity\t$identity_lixin\n";
	...
	if(!defined($best{"$name\t$t_name"}) || $best{"$name\t$t_name"} < $best)
	{
		$best{"$name\t$t_name"} = $best;
		$best_result{"$name\t$t_name"} = $_;
		$best_tab{"$name\t$t_name"} = "$name\t$t_name\t$rough\t$cutoff\t$identity";
		$best_id{"$name\t$t_name"}="$name\t$t_name\t$cutoff";
	}
}
