...
while (<Solar>) {
	...
	if ($exon_num{$infor[0]}!=1){next;}
	if ($exon_num{$infor[5]}==1) {if($rough/$infor[1]>0.6){$char{$infor[0]}=1};next;}
	#if (!exists $exon_num{$infor[5]}){print ">>>>>>>>>>>>>>>>>>>>>>>>>$infor[5]\n";}
	my @tmp = split/[\,\;]/,$infor[12];
	unshift(@tmp,"1");
	@tmp = &cat(@tmp);
	my $count=0;
	for (my $i=0;$i<@tmp-1 ;$i+=2) {#query的每一个blcok和database的gene结构作比较
		$align_start{$infor[5]}->[$count]=$tmp[$i];
		$align_end{$infor[5]}->[$count]=$tmp[$i+1];
		
		for (my $j=0;$j<$block_num{$infor[5]} ;$j++) {#一个exon一个exon的比
			if (!($align_start{$infor[5]}->[$count]>$block_end{$infor[5]}->[$j] or $align_end{$infor[5]}->[$count]<$block_start{$infor[5]}->[$j])) 
			{#若和其中一个exon有overlap，计算overlap的长度
				
				if ($align_start{$infor[5]}->[$count]>=$block_start{$infor[5]}->[$j] and $align_end{$infor[5]}->[$count]<=$block_end{$infor[5]}->[$j])
				{$overlap_len=$align_end{$infor[5]}->[$count]-$align_start{$infor[5]}->[$count]+1;}#比对的区域完全落入一个exon的情况
				
				elsif($align_start{$infor[5]}->[$count]<=$block_end{$infor[5]}->[$j] and $align_end{$infor[5]}->[$count]>=$block_end{$infor[5]}->[$j])
				{$overlap_len=$block_end{$infor[5]}->[$j]-$align_start{$infor[5]}->[$count]+1;}
				
				elsif($align_start{$infor[5]}->[$count]<=$block_start{$infor[5]}->[$j] and $align_end{$infor[5]}->[$count]>=$block_start{$infor[5]}->[$j])
				{$overlap_len=$align_end{$infor[5]}->[$count]-$block_start{$infor[5]}->[$j]+1;}
				
				elsif($align_start{$infor[5]}->[$count]<=$block_start{$infor[5]}->[$j] and $align_end{$infor[5]}->[$count]>=$block_end{$infor[5]}->[$j])
				{$overlap_len=$block_end{$infor[5]}->[$j]-$block_start{$infor[5]}->[$j]+1;}
				
				else {print "Wrong!!!!!!!!!!!!!!!!!!!!!!\n";die '!';}
				if ($overlap_len>=20) {#overlap的长度超过20bp，认为比到了一个exon
					$align_exon_num{"$block_start{$infor[5]}->[$j]\t$block_end{$infor[5]}->[$j]"}=1;#防止一个exon分2段比到同一个exon误判的情况
					print "$infor[0]\t$infor[5]\t",$align_start{$infor[5]}->[$count],"\t",$align_end{$infor[5]}->[$count],"\t",$block_start{$infor[5]}->[$j],"\t",$block_end{$infor[5]}->[$j],"\n";
				}
			}
		}
	...
	if ((keys %align_exon_num)>1) {#若比到>=2个exon，则输出
		if (!exists $char{$infor[0]}){$char{$infor[0]}=0;}
		print OUT "$infor[0]\t$infor[1]\t$infor[2]\t$infor[3]\t$infor[4]\t$infor[5]\t";
		for (my $i=6;$i<=13;$i++){print OUT "$infor[$i]\t";}
		print OUT "$char{$infor[0]}\n";
	}
	#last;
